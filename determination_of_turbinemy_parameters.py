...
class RegularTurbin:
    ...
    def vibration_diagram(self, m, t, beta, density, E, z, d, l, f, J, delta, B, psi):
        """
        Расчет и построение вибрационной диаграммы лопаток
        
        Параметры:
            m - количество лопаток
            t - шаг лопаток, м
            beta - угол установки лопаток, градусы
            density - плотность материала лопатки, кг/м3
            E - модуль упругости материала, Па
            z - количество бандажных полок
            d - диаметр ротора, м
            l - длина лопатки, м
            f - площадь поперечного сечения лопатки, м2
            J - момент инерции сечения, м4
            delta - толщина бандажной полки, м
            B - ширина бандажной полки, м
            psi - коэффициент формы сечения
        """
        mm = 1 / 1000
        i = (J / f) ** 0.5
        _lambda = l / i
        ...
        ax.grid(True)
        ax.legend()
        ax.set_title("Вибрационная диаграмма лопаток турбины")
    ...
    def plot_smoothed_profile(self, xu, yu, xl, yl, num_points=100, show_plot=True):
        """
       
        
        Параметры:
            xu, yu - координаты верхнего профиля
            xl, yl - координаты нижнего профиля
            num_points - количество точек для интерполяции
            show_plot - показывать график сразу (True) или только возвращать fig (False)
        """
        ...
        # Построение графиков
        ax.plot(x_u_smooth, y_u_smooth, 'b-', label='Верхний профиль (сглаженный)')
        ax.plot(x_l_smooth, y_l_smooth, 'r-', label='Нижний профиль (сглаженный)')
        ax.scatter(xu, yu, c='blue', s=40, label='Исходные точки (верх)')
        ax.scatter(xl, yl, c='red', s=40, label='Исходные точки (низ)')
        
        # Настройки графика
        ax.set_aspect('equal')
        ax.grid(True, linestyle='--', alpha=0.7)
        ax.legend()
        ax.set_title("Профиль рабочей лопатки")
        ax.set_xlabel("X [м]")
        ax.set_ylabel("Y [м]")
    ...
    def create_3d_blade(xu, yu, xl, yl, length, num_slices=50):
        """Создает 3D-модель лопатки"""
        # Интерполяция профиля
        f_u = interp1d(xu, yu, kind='cubic')
        f_l = interp1d(xl, yl, kind='cubic')
        
        # Создаем сетку по длине лопатки
        z_coords = np.linspace(0, length, num_slices)
        x_u = np.tile(xu, (num_slices, 1))
        z_u = np.tile(z_coords.reshape(-1, 1), (1, len(xu)))
        y_u = np.array([f_u(x) for x in xu])
        y_u = np.tile(y_u, (num_slices, 1))
        
        x_l = np.tile(xl, (num_slices, 1))
        z_l = np.tile(z_coords.reshape(-1, 1), (1, len(xl)))
        y_l = np.array([f_l(x) for x in xl])
        y_l = np.tile(y_l, (num_slices, 1))
        
        return (x_u, y_u, z_u), (x_l, y_l, z_l)
    
    def calculate_stresses(z_coords, diameter, blade_length, angular_speed, rho):
        """Расчет напряжений по длине лопатки"""
        r_root = (diameter - blade_length)/2
        stresses = []
        
        for z in z_coords:
            term1 = r_root * (blade_length - z)
            term2 = 0.5*(blade_length**2 - z**2)
            sigma = rho * angular_speed**2 * (term1 + term2)
            stresses.append(sigma/1e6)  # в МПа
    ...
    """Визуализация 3D модели лопатки с распределением напряжений"""
    try:
        # Проверка входных данных
        assert len(xu) == len(yu), "xu и yu должны иметь одинаковую длину"
        assert len(xl) == len(yl), "xl и yl должны иметь одинаковую длину"
        assert len(stress_distribution) > 1, "Нужно минимум 2 значения напряжений"
        
        # Интерполяция профилей
        f_u = interp1d(xu, yu, kind='cubic', fill_value='extrapolate')
        f_l = interp1d(xl, yl, kind='cubic', fill_value='extrapolate')
        ...
        # Координаты по высоте лопатки
        z_coords = np.linspace(0, height, num_points)
        
        # Интерполяция напряжений
        stress_interp = interp1d(
            np.linspace(0, height, len(stress_distribution)),
            stress_distribution,
            kind='linear',
        ...
        fig.update_layout(
            title='3D Визуализация напряжений в лопатке',
            scene=dict(
                xaxis_title='Ширина, м',
                yaxis_title='Толщина, м',
                zaxis_title='Длина, м'
            )
        )
        ...
        print(f"Ошибка в plot_3d_blade_with_stress: {str(e)}")
        raise
...
class RegularTurbin:
    ...
    def plot_smoothed_profile(self, xu, yu, xl, yl, num_points=100, show_plot=True):
        ...
        # Построение графиков
        ax.plot(x_u_smooth, y_u_smooth, 'b-', label='Верхний профиль ')
        ax.plot(x_l_smooth, y_l_smooth, 'r-', label='Нижний профиль ')
        ax.scatter(xu, yu, c='blue', s=40, label='Исходные точки ')
        ax.scatter(xl, yl, c='red', s=40, label='Исходные точки')
